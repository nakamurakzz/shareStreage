...
type Props = { article:{title:string,url:string,content:string} }
...
export const getStaticPaths:GetStaticPaths=async()=>{
  const response = await fetch('https://newsapi.org/v2/everything?q=bitcoin&apiKey=64eae1a0688d48fbbfb89d6c9fc026b1')
  const responseJson:ArticleResponse = await response.json()

  // IDが取得できるAPIの方が望ましいと思われる。map関数のindexで無理やり採番している
  const paths = responseJson.articles.map((_article,index) => `/articles/${index+1}`)
  console.table(paths)
  return {paths,fallback:false} // fallback:false で他のパスは404
}

export const getStaticProps:GetStaticProps=async({params})=> {
  // paramsはリクエスト時のパスパラメータで定義される
  if(params==undefined)throw new Error();
  console.table(params)
  // テスト用の定数
  const response = await fetch('https://newsapi.org/v2/everything?q=bitcoin&apiKey=64eae1a0688d48fbbfb89d6c9fc026b1')
  const responseJson:ArticleResponse = await response.json()
  const article = await responseJson.articles[Number(params.id)-1]
  // posts変数がArticleメソッドに渡される
  return {
    props: {
      article,
    },
